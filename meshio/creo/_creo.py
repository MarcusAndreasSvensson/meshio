    ...
    while 1:
        if mode == "header":
            ...
            for it in range(n_types):
                ...
                # mesh.cells_dict[t_name] = aux
                if t_name == "tet":
                    cell_list.append(("tetra", aux))
                elif t_name == "prism":
                    cell_list.append(("wedge", aux))
                elif t_name == "tri":
                    cell_list.append(("triangle", aux))
                elif t_name == "vtx":
                    cell_list.append(("vertex", aux))
                elif t_name == "edg":
                    cell_list.append(("line", aux))
                else:
                    cell_list.append((t_name, aux))

            break

    # mesh._set_io_data(coors, None, conns, mat_ids, descs)

    cells = [CellBlock(name, data) for (name, data) in cell_list]

    mesh = Mesh(coors, cells)
...
register("creo", [".prt"], read, {"prt": write})
